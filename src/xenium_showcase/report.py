...
def build_html_report(result, outdir, cfg) -> Path:
    ...
    captions = {
        ...
        "11_anomaly_map.png": "Isolation Forest flags joint multivariate outliers for review; calls are advisory by default.",
    }
    figures = {}
    ...
    auto_qc = summary.get("automatic_qc", {})
    evaluation = result.model_evaluation.get("annotation", {})
    failure_counts = pd.DataFrame(
        list(auto_qc.get("rule_failure_counts", {}).items()), columns=["QC failure mode", "cells"]
    )
...
<h2>Automatic QC and anomaly detection</h2><section><p>Hard rules remain the primary, interpretable inclusion decision. Isolation Forest evaluates joint abnormalities across available yield, control, area, nucleus/cell-ratio and transcript-density features. It flagged <b>{auto_qc.get("anomaly_count_among_rule_pass", 0):,}</b> otherwise rule-passing cells. Model calls are {'excluded from' if auto_qc.get('anomalies_excluded') else 'advisory and do not alter'} the primary analysis set.</p>{_table(failure_counts, 20)}</section><div class="grid">{group(["11_anomaly_map.png"])}</div>
<h2>Model evaluation</h2><section><p>Resolved fraction: <b>{evaluation.get("resolved_fraction", float("nan")):.1%}</b>. Cluster/annotation normalized mutual information among resolved cells: <b>{evaluation.get("cluster_label_nmi_resolved", float("nan")):.3f}</b>. These are internal consistency diagnostics, not accuracy estimates; no pathology ground truth or independent reference labels are available.</p></section>
<h2>Limitations and failure modes</h2><section><ul><li>Isolation Forest detects statistical rarity, not invalid biology; rare valid states may be flagged and common artifacts may be missed.</li><li>Marker-module labels are panel-dependent working annotations. Unresolved cells combine missing evidence, mixed states, segmentation contamination and real transitional biology.</li><li>KNN enrichment depends on k, sampling and tissue geometry; label permutation does not demonstrate causal interaction.</li><li>Moran's I reports spatial coherence, not gene-wise significance or causality.</li><li>A single patient/section cannot support patient-level inference; cells are not independent biological replicates.</li></ul><p>See <code>docs/limitations_and_failure_modes.md</code> for the complete detection and mitigation register.</p></section>
